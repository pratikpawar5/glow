...
//Add Service Category From gloqr-admin
@Component({
...
export class AddServiceCategoryComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.categoryForm.valid) {
      if (this.newCategory) {
        ...
        if (valid) {
          this.http.post<CustomHttpResponse<any>>(environment.SERVICE_URL + 'admin/categories', this.categoryForm.value, {
        ...
        this.showButton = false
        this.http.post<CustomHttpResponse<any>>(environment.SERVICE_URL + 'admin/categories/'
  ...
  uploadImagesToServer(file: File, index: number, type: string) {
    ...
    this.http.post<CustomHttpResponse<any>>(environment.SERVICE_URL + 'file', formData, {
      headers: this.headers
    }).subscribe(
      res => {
        if (!res.error) {
  ...
  getCategories() {
    this.http.get<CustomHttpResponse<any>>(environment.SERVICE_URL + 'categories', {
      headers: this.headers
    }).subscribe(res => {
      if (!res.error) {
        this.serviceCategories = res.data.categories
      }
    })
  }
